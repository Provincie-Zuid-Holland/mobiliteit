---
layout: pzh
title: Veranderingen draaien om gedrag
header_image: /mobiliteit/kansen/img/kop-gedrag.jpg
focus: 50% 15%
menu: |
    <a href="/mobiliteit/kansen/index.html">Negen kansen voor bereikbaarheid</a>
    <a href="/mobiliteit/snelstudie/disclaimer.html">disclaimer</a>

---
<p class="ondertitel">Bij alle negen kansen speelt gedrag een hoofdrol</p>

Om de kansen voor mobiliteit optimaal te benutten en effectief beleid te maken, moeten we in alle stadia van beleidsvorming rekening houden met menselijk gedrag. We moeten ons bewust zijn dat mensen zich niet altijd rationeel gedragen en dat er geen verandering kan plaatsvinden zonder mensen mee te krijgen.

Nu vinden we het dragen van een autogordel normaal en het roken van sigaretten ongezond. Dat was vroeger anders. Het maatschappelijke beeld van wat normaal en wat belangrijk is, verandert voortdurend. Daarmee verandert ook ons gedrag. Soms gaat dat vanzelf, maar soms kunnen we (de overheid, bedrijven en particulieren) een handje helpen. Denk aan grote gedragscampagnes zoals BOB of Nix \<18 die ons collectieve alcoholgedrag veranderd hebben. Maar ook op kleine(re) schaal kan gedrag veranderd worden.

Gedragsverandering kan ook plaatsvinden doordat alternatieven de 'makkelijkere' keuze worden. Bedenk je maar eens wat er zou gebeuren als biologisch vlees goedkoper zou worden dan kiloknallers. Of als je route naar werk, door een nieuwe bushalte in je buurt en gratis OV, een stuk makkelijker en goedkoper wordt dan met de auto.

<div class="voorbeeld voorbeeld--geel" markdown="1">

## Samen houden we Zuid-Holland bereikbaar

Het wordt steeds drukker in Zuid-Holland. De komende jaren worden zo'n 200 duizend extra woningen gebouwd. Ook hebben veel wegen, bruggen en tunnels groot onderhoud nodig. Dat leidt tot extra drukte op de wegen, op het water en in het openbaar vervoer. Als we niets doen, staan we in Zuid-Holland nog vaker stil. Daarom bundelen negen publieke organisaties hun krachten in de samenwerkingsorganisatie [Zuid-Holland Bereikbaar](https://zuidhollandbereikbaar.nl/). Kijk op de site wat ze doen en hoe je mee kunt doen.

</div>

## Wat is gedrag?

-   Alle acties en reacties van een persoon in relatie tot haar omgeving.
-   In de gedragsaanpak focussen we alleen op zichtbaar, waarneembaar gedrag: alles wat je kunt voordoen en nadoen. Hoe concreter het gedragsdoel, hoe beter.
-   Fietsen, autorijden, voertuigen delen, bijna alle keuzes voor mobiliteit zijn gedrag.

## Hoe werkt gedragsverandering?

Het simpele antwoord is: maak het gewenste gedrag zo makkelijk mogelijk en het ongewenste gedrag zo moeilijk mogelijk. Maar voordat je een maatregel bedenkt, is het belangrijk om te begrijpen waarom iemand bepaald gedrag wel of niet vertoont.

Het COM-B model helpt daarbij. COM-B staat voor Capability, Opportunity en Motivation → Behaviour: gedrag ontstaat uit een combinatie van capaciteit, gelegenheid en motivatie.

<figure>
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 430" role="img"
     aria-label="Het COM-B model: capaciteit, gelegenheid en motivatie leiden samen tot gedrag. Capaciteit en gelegenheid werken ook op de motivatie in."
     style="width:100%;height:auto;display:block">
  <defs>
    <marker id="comb-pijl" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
      <path d="M0 0 L10 5 L0 10 z" fill="#281f6b"/>
    </marker>
  </defs>
  <g fill="none" stroke="#281f6b" stroke-width="3" marker-end="url(#comb-pijl)">
    <path d="M274 160 L368 197"/>
    <path d="M274 260 L368 223"/>
    <path d="M530 210 L647 210"/>
    <path d="M256 64 Q480 6 676 146"/>
    <path d="M256 356 Q480 414 676 274"/>
  </g>
  <g stroke="#281f6b" stroke-width="3">
    <circle cx="200" cy="120" r="80" fill="#e5eff8"/>
    <circle cx="200" cy="300" r="80" fill="#e5eff8"/>
    <circle cx="450" cy="210" r="80" fill="#e5eff8"/>
    <circle cx="740" cy="210" r="85" fill="#efcc36"/>
  </g>
  <g font-family="Karbon, Arial, Helvetica, sans-serif" text-anchor="middle" fill="#281f6b" font-weight="600" font-size="25">
    <text x="200" y="113">Capaciteit</text>
    <text x="200" y="293">Gelegenheid</text>
    <text x="450" y="203">Motivatie</text>
    <text x="740" y="221" font-size="30">Gedrag</text>
  </g>
  <g font-family="Karbon, Arial, Helvetica, sans-serif" text-anchor="middle" fill="#16113b" font-size="18">
    <text x="200" y="142">kan ik het?</text>
    <text x="200" y="322">kan het hier?</text>
    <text x="450" y="232">wil ik het?</text>
  </g>
</svg>
  <figcaption>Het COM-B model -- naar: Michie, Van Stralen &amp; West, <a href="https://doi.org/10.1186/1748-5908-6-42">The behaviour change wheel</a> (Implementation Science, 2011)</figcaption>
</figure>


### Capaciteit -- kan iemand het gewenste gedrag uitvoeren?

Capaciteit gaat over wat iemand lichamelijk en mentaal kan. Denk bijvoorbeeld aan weten hoe je met het openbaar vervoer reist, kunnen fietsen, of voldoende conditie hebben om een bepaalde afstand te fietsen. Als iemand de benodigde capaciteit mist, is het lastig om ander gedrag te verwachten. We kunnen dan bijvoorbeeld kennis en vaardigheden vergroten, of een alternatief bieden dat wel haalbaar is.

### Gelegenheid -- maakt de omgeving het gewenste gedrag mogelijk en aantrekkelijk?

De omgeving bepaalt in belangrijke mate welke keuzes makkelijk of juist lastig zijn. Daarbij gaat het niet alleen om de fysieke omgeving, zoals fietspaden, stallingen, parkeerplaatsen en openbaar vervoer, maar ook om de sociale omgeving. Wat doen bijvoorbeeld je collega's, vrienden of buren? Als iedereen met de auto naar het werk gaat, kan dat de vanzelfsprekende keuze worden. Als fietsen goed gefaciliteerd wordt en collega's regelmatig de fiets pakken, wordt dat gedrag juist makkelijker en normaler.

### Motivatie -- wil iemand het gewenste gedrag uitvoeren?

Motivatie gaat over de bewuste én automatische processen die gedrag beïnvloeden. Iemand kan bijvoorbeeld bewust kiezen voor de fiets omdat die gezond en duurzaam is, maar tegelijkertijd automatisch de auto pakken omdat dat vertrouwd en comfortabel is. Met communicatie, feedback, beloningen en andere interventies kunnen we motivatie beïnvloeden.

### Van analyse naar aanpak

Het COM-B model helpt ons dus eerst te bepalen waarom mensen bepaald mobiliteitsgedrag vertonen. Pas daarna kijken we welke aanpak daarbij past. Soms ligt de oplossing in betere infrastructuur of voorzieningen, soms in het vergroten van kennis en vaardigheden en soms in het beïnvloeden van motivatie.

Vaak is een combinatie nodig. Want als we willen dat mensen vaker fietsen, is alleen vertellen dat fietsen gezond en duurzaam is niet genoeg als er geen veilige fietsroute is. En een goede fietsinfrastructuur leidt niet vanzelf tot meer fietsers als mensen niet weten welke route ze kunnen nemen of fietsen niet als een aantrekkelijke optie zien.

Kortom: effectief werken aan gedragsverandering begint niet bij de interventie, maar bij het begrijpen van het gedrag.

Op deze pagina staan een paar voorbeelden van hoe je op het gebied van mobiliteit gedrag kunt veranderen.

<div class="voorbeeld" markdown="1">

## Stimuleer fietsgebruik vanuit werkgevers

Werkgevers kunnen het gebruik van de fiets door hun werknemers stimuleren. Dat kan bijvoorbeeld met [fiscaal voordeel](https://www.rijksoverheid.nl/onderwerpen/fiets/vraag-en-antwoord/fiets-van-de-zaak), voorzieningen (fietsenstalling, oplaadpunten, omkleedruimte, douche) en een [spaarsysteem](https://trappersshop.fiscfree.nl/). Zo kunnen bedrijven hun werknemers een fiscaalvriendelijke ‘fiets van de zaak’ geven. Dat mag ook een e-bike of speed-pedelec zijn. De werkgever mag belastingvrij 25 cent per fietskilometer vergoeden. Extra lichaamsbeweging houdt werknemers vitaal: dat vertaalt zich vaak naar een lager ziekteverzuim. Ook carpoolen (samen reizen) komt in aanmerking voor diezelfde vergoeding. De samenwerkingsorganisatie [Zuid-Holland Bereikbaar](https://www.zuidhollandbereikbaar.nl/) van overheden en bedrijfsleven kan hierbij helpen. Voor werkgevers heeft de Fietsersbond een [checklist fietsvriendelijk bedrijf](https://www.fietsersbond.nl/onderweg/fietsen-naar-het-werk/fietsvriendelijk-bedrijf/checklist-fietsvriendelijk-bedrijf/).

</div>


<div class="voorbeeld voorbeeld--media voorbeeld--half">
<div class="video"><iframe src="https://www.youtube-nocookie.com/embed/jSeJ4BOKww8" title="Leefstraten in Gent" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>
<div markdown="1">

## Experimenteer met leefstraten

Bij een leefstraat wordt een straat of plein een aantal maanden 'teruggegeven' aan de bewoners en wordt hij ingericht als veilige en rustige ontmoetingsplaats. Het gaat om locaties waar veel mensen wonen, maar elkaar ontmoeten niet altijd vanzelf spreekt. Bewoners kunnen bankjes neerzetten, uitgebreid met elkaar kletsen terwijl kinderen veilig kunnen spelen. Zie ook deze video over leefstraten in Gent.

</div>
</div>


<div class="voorbeeld voorbeeld--media voorbeeld--half">
<div class="video"><iframe src="https://player.vimeo.com/video/379028081?dnt=1" title="Maak kennis met de OV-ambassadeurs van Ervaar het OV" loading="lazy" allowfullscreen></iframe></div>
<div markdown="1">

## Stimuleer OV met campagnes

Het OV kan gestimuleerd worden door het onder de aandacht te brengen met campagnes (regionaal of landelijk) en marketing (probeeracties, proefabonnementen, reisadviezen). De provincie Gelderland en de regionale vervoerders voeren een meerjarige campagne (met de website Ervaar het OV) om inwoners te verleiden het OV te proberen. Uit onderzoek blijkt dat mensen openstaan voor andere vormen van reizen bij grote veranderingen, zoals een nieuwe woonplaats, ander werk of langdurige wegwerkzaamheden.

</div>
</div>

## Verder lezen

{% include leesmeer.html kop="Lees meer in het interview" title="Gewoontes doorbreken voor een groene wereld" tekst="Klimaatpsycholoog Gerdien de Vries (TU Delft) onderzoekt waarom we duurzaam gedrag wel belangrijk vinden, maar oude gewoontes toch moeilijk loslaten." url="https://www.tudelft.nl/stories/articles/gewoontes-doorbreken-voor-een-groene-wereld" image="/mobiliteit/kansen/img/eerdere/gerdien-de-vries.jpg" %}

{% include leesmeer.html title="Hoe verleiden we reizigers om slimmer (dan met een auto) te reizen?" url="https://www.zuid-holland.nl/publish/pages/27681/hoeverleidenwereizigersomslimmertereizen.pdf" image="/mobiliteit/kansen/img/eerdere/slimmer-reizen.jpg" %}

<p class="terug"><a href="/mobiliteit/kansen/index.html">← terug naar de negen kansen</a></p>
